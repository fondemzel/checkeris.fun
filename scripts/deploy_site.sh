...
#
#   scripts/deploy_site.sh "Карточка товара в правой панели"   # 0.4.1 → 0.5.0
#   scripts/deploy_site.sh --patch "Опечатка в подписи"        # 0.5.0 → 0.5.1
#   scripts/deploy_site.sh --major "Личные кабинеты"           # 0.5.1 → 1.0.0
...
#
# КАЖДАЯ ВЫКЛАДКА ПОДНИМАЕТ ВЕРСИЮ. По умолчанию minor (0.4.1 → 0.5.0) — даже мелкие
# правки идут минором; --patch для совсем косметики, --major для несовместимых изменений.
...
Версия поднимается на каждой выкладке:
  без флага   minor  0.4.1 → 0.5.0   правки, улучшения, новые функции
  --patch            0.5.0 → 0.5.1   косметика, если минор избыточен
  --major            0.5.1 → 1.0.0   несовместимые изменения
TEXT
}
