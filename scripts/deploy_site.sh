...
#
#   scripts/deploy_site.sh "Карточка товара в правой панели"   # 0.1.0 → 0.1.1
#   scripts/deploy_site.sh --minor "Раздел категорий"          # 0.1.1 → 0.2.0
#   scripts/deploy_site.sh --major "Личные кабинеты"           # 0.2.0 → 1.0.0
...
#
# КАЖДАЯ ВЫКЛАДКА ПОДНИМАЕТ ВЕРСИЮ. По умолчанию patch (0.1.0 → 0.1.1);
...
Версия поднимается на каждой выкладке:
  без флага   patch  0.1.0 → 0.1.1   правки, мелкие улучшения
  --minor            0.1.1 → 0.2.0   новый раздел или заметная функция
  --major            0.2.0 → 1.0.0   несовместимые изменения
TEXT
}
